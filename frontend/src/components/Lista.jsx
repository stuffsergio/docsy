...
import { Ellipsis, Pencil, Trash, ArrowRight } from "lucide-react";
...
export default function Lista({
  ...
  return (
    <li className="flex flex-col justify-between gap-4 py-8 px-6 border border-[#1f1f1f]">
      <div className="flex flex-row items-start gap-0.5">
        <div className="flex flex-col">
          ...
          {/* BODY */}
          <div className="mt-4">
            <p className="text-sm opacity-85 pr-2.5 md:line-clamp-3">{body}</p>
          </div>
        </div>
        <p className="geist-mono pr-2.5 text-xl">{index + 1}</p>
      </div>
          ...
          <div className="flex flex-row items-center gap-3">
            <div>
              <img
                ...
                className="w-6 h-auto rounded-full border border-white"
              />
            </div>
            <div className="flex flex-row items-center gap-1.5">
              <p className="text-xs opacity-70">Creado por {usuario?.name}</p>
              {usuario?.role === "admin" && (
      ...
      {location.pathname === "/docs" && (
        <div className="flex flex-row justify-between items-center pt-4 pr-4">
          ...
          >
            <span className="relative" onMouseEnter={() => setShowMenu(true)}>
            ...
            {showMenu && (
              ...
              >
                ...
                <motion.button onClick={setQuienEliminar} {...whileAnimation}>
